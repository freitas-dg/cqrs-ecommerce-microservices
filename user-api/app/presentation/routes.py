...
def init_dependencies(cache: RedisCache, search: ElasticsearchAdapter, publisher: RabbitMQPublisher, encryption: EncryptionService) -> None:
    global _cache, _search, _publisher, _encryption
    _cache = cache
    _search = search
...
def _get_use_cases(session: AsyncSession=Depends(get_db_session)) -> UserUseCases:
    ...
    return UserUseCases(repository=repo, cache=_cache, search=_search, event_publisher=_publisher)
...
async def create_user(body: UserCreateRequest, use_cases: UserUseCases=Depends(_get_use_cases)):
    try:
        user = await use_cases.create_user(name=body.name, cpf=body.cpf, email=body.email, phone_number=body.phone_number)
...
@router.get('/search', response_model=List[dict], summary='Search users via Elasticsearch')
